...
export default function Users() {
  ...
  const getUsers = async (location = undefined) => {
    let url = location
      ? `/users/followers/${location.toLowerCase()}`
      : '/users/followers'
  ...
  return (
    <Layout pageName="User - Followers Leaderboard" sidebar={false}>
      <h1>User Followers Leaderboards</h1>
      <p>
        Daftar leaderboard pengguna-pengguna dengan jumlah pengikut terbanyak di
        Github.
      </p>

      <h2>Global</h2>
      <p>
        Top {usersGlobal.length} pengguna Github dengan jumlah pengikut
      ...
      <p>
        ...
        pengikut terbanyak di <strong>{usersByLocation.location}</strong> {date}
        .
      </p>
