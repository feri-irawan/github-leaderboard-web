...
export default function Users() {
  ...
  const getUsers = async (location = undefined) => {
    let url = location
      ? `/users/commits/${location.toLowerCase()}`
      : '/users/commits'
  ...
  return (
    <Layout pageName="User - Commits Leaderboard" sidebar={false}>
      <h1>User Commits Leaderboard</h1>
      <p>
        Daftar leaderboard pengguna dengan jumlah commit terbanyak di Github.
      </p>

      <h2>By Location</h2>
      <p>
        Top {usersByLocation.users.length} pengguna Github dengan jumlah commit
        terbanyak di <strong>{usersByLocation.location}</strong> {date}.
      </p>
